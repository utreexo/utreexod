...
func getBlockFromString(hexStr string) *btcutil.Block {
	serializedBlock, err := hex.DecodeString(hexStr)
	if err != nil {
		// It's ok since we're only parsing hard-coded blocks.
		err := fmt.Errorf("couldn't decode hex string of %s", hexStr)
		panic(err)
	}
	...
	if err != nil {
		// It's ok since we're only parsing hard-coded blocks.
		err := fmt.Errorf("couldn't make a new block from bytes. Decoded hex string:  %s", hexStr)
		panic(err)
	}
...
func compareMultipleChainTips(gotChainTips, expectedChainTips []*btcjson.GetChainTipsResult) error {
	...
	for _, expectedChainTip := range expectedChainTips {
		...
		if err != nil {
			return fmt.Errorf("Expected chainTip with hash %s differ. %s", expectedChainTip.Hash, err)
		}
	}
...
// compareChainTips is a helper function to compare two chain tips against one another.
// Returns an error if they're different.
func compareChainTips(got, expected btcjson.GetChainTipsResult) error {
	if got.Height != expected.Height {
		return fmt.Errorf("Expected height of %d but got %d", expected.Height, got.Height)
	}
	if got.Hash != expected.Hash {
		return fmt.Errorf("Expected hash of %s but got %s", expected.Hash, got.Hash)
	}
	if got.BranchLen != expected.BranchLen {
		return fmt.Errorf("Expected branchlen of %d but got %d", expected.BranchLen, got.BranchLen)
	}
	if got.Status != expected.Status {
		return fmt.Errorf("Expected status of %s but got %s", expected.Status, got.Status)
	}
...
func TestGetChainTips(t *testing.T) {
	...
	err = compareMultipleChainTips(gotChainTips, expectedChainTips)
	if err != nil {
	...
	for _, blockString := range blockStrings {
		block := getBlockFromString(blockString)
	...
	expectedChainTips = []*btcjson.GetChainTipsResult{
		{
			Height:    4,
			Hash:      getBlockFromString(blockStrings[len(blockStrings)-1]).Hash().String(),
			BranchLen: 0,
			Status:    "active",
		},
	}
	...
	expectedChainTips = []*btcjson.GetChainTipsResult{
		{
			Height:    4,
			Hash:      getBlockFromString(block4Hex).Hash().String(),
			BranchLen: 0,
			Status:    "active",
		},
		{
			Height:    3,
			Hash:      getBlockFromString(block3aHex).Hash().String(),
			BranchLen: 2,
			Status:    "valid-fork",
		},
	}
	...
	// (genesis block) -> 1 -> 2  -> 3  -> 4   (active)
	//                    \ -> 2a -> 3a -> 4a  (valid-fork)
	block := getBlockFromString(block4aHex)
	err = r.Client.SubmitBlock(block, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	expectedChainTips = []*btcjson.GetChainTipsResult{
		...
		{
			Height:    4,
			Hash:      getBlockFromString(block4aHex).Hash().String(),
			BranchLen: 3,
			Status:    "valid-fork",
		},
	}
	...
	//                    \ -> 2a -> 3a -> 4a -> 5a  (active)
	block = getBlockFromString(block5aHex)
	err = r.Client.SubmitBlock(block, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	expectedChainTips = []*btcjson.GetChainTipsResult{
		...
		{
			Height:    5,
			Hash:      getBlockFromString(block5aHex).Hash().String(),
			BranchLen: 0,
			Status:    "active",
		},
	}
	...
	//                    \ -> 2a -> 3a -> 4a -> 5a  (active)
	//                                \ -> 4b        (valid-fork)
	block = getBlockFromString(block4bHex)
	err = r.Client.SubmitBlock(block, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	expectedChainTips = []*btcjson.GetChainTipsResult{
		...
		{
			Height:    4,
			Hash:      getBlockFromString(block4bHex).Hash().String(),
			BranchLen: 1,
			Status:    "valid-fork",
		},
	}
