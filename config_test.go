...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-utreexod.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "utreexod")
	if err != nil {
	...
	tmpConfigFile := filepath.Join(appPath, "sample-utreexod.conf")
