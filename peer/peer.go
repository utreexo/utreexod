...
func (p *Peer) negotiateInboundProtocol() error {
	...
	if p.cfg.UsingV2Conn {
		...
		err := p.V2Transport.RespondV2Handshake(
			garbageLen, v2transport.BitcoinNet(p.cfg.ChainParams.Net),
		)
		switch {
		...
		default:
			err = p.V2Transport.CompleteHandshake(
				false, nil, v2transport.BitcoinNet(p.cfg.ChainParams.Net),
			)
			if err != nil {
				return err
			}
...
func (p *Peer) negotiateOutboundProtocol() error {
	if p.cfg.UsingV2Conn {
		...
		err = p.V2Transport.CompleteHandshake(
			true, nil, v2transport.BitcoinNet(p.cfg.ChainParams.Net),
		)
...
// NewOutboundPeer returns a new outbound bitcoin peer. If connecting to an
// onion hidden service or a hostname, the caller should set HostToNetAddress
// in the Config to avoid a nil-pointer-dereference.
