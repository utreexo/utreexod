...
utreexod provides a [JSON-RPC](http://json-rpc.org/wiki/specification) API that is
fully compatible with the original bitcoind/bitcoin-qt.  There are a few key
differences between utreexod and bitcoind as far as how RPCs are serviced:
* Unlike bitcoind that has the wallet and chain intermingled in the same process
  which leads to several issues, utreexod intentionally splits the wallet and chain
  ...
  talking directly to utreexod, only chain-related RPCs are available.  However both
  chain-related and wallet-related RPCs are available via
  [btcwallet](https://github.com/btcsuite/btcwallet).
* utreexod is secure by default which means that the RPC connection is TLS-enabled
  by default
* utreexod provides access to the API through both
  [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29) requests and
  [Websockets](http://en.wikipedia.org/wiki/WebSocket)

Websockets are the preferred transport for utreexod RPC and are used by applications
such as [btcwallet](https://github.com/btcsuite/btcwallet) for inter-process
communication with utreexod.  The websocket connection endpoint for utreexod is
...
The utreexod RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
utreexod is `wss://your_ip_or_domain:8334/ws`.
...
to a utreexod RPC server:

* **rpcuser** is the full-access username configured for the utreexod RPC server
* **rpcpass** is the full-access password configured for the utreexod RPC server
* **rpclimituser** is the limited username configured for the utreexod RPC server
* **rpclimitpass** is the limited password configured for the utreexod RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the utreexod
  server is configured with.  It is automatically generated by utreexod and placed
  in the utreexod home directory (which is typically `%LOCALAPPDATA%\utreexod` on
  Windows and `~/.utreexod` on POSIX-like OSes)

**NOTE:** As mentioned above, utreexod is secure by default which means the RPC
...
The utreexod RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
utreexod comes with a separate utility named `utreexoctl` which can be used to issue
these RPC commands via HTTP POST requests to utreexod after configuring it with the
...
|26|[sendrawtransaction](#sendrawtransaction)|Y|Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><font color="orange">utreexod does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
|27|[setgenerate](#setgenerate) |N|Set the server to generate coins (mine) or not.<br/>NOTE: Since utreexod does not have the wallet integrated to provide payment addresses, utreexod must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|28|[stop](#stop)|N|Shutdown utreexod.|
...
|31|[validateaddress](#validateaddress)|Y|Verifies the given address is valid.  NOTE: Since utreexod does not have a wallet integrated, utreexod will only return whether the address is valid or not.|
...
|Notes|NOTE: Since utreexod does NOT contain wallet functionality, wallet-related fields are not returned.  See getinfo in btcwallet for a version which includes that information.|
...
|Notes|<font color="orange">Since utreexod does not perform any mining, the priority related fields `startingpriority` and `currentpriority` that are available when the `verbose` flag is set are always 0.</font>|
...
|Notes|NOTE: Since utreexod does not have the wallet integrated to provide payment addresses, utreexod must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|Returns|Nothing|
[Return to Overview](#MethodOverview)<br />
...
|Notes|<font color="orange">utreexod does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown utreexod.|
|Returns|`"utreexod stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
|Description|Verifies the block chain database.<br />The actual checks performed by the `checklevel` parameter is implementation specific.  For utreexod this is:<br />`checklevel=0` - Look up each block and ensure it can be loaded from the database.<br />`checklevel=1` - Perform basic context-free sanity checks on each block.|
|Notes|<font color="orange">utreexod currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for utreexod.</font>|
...
The following is an overview of the RPC methods which are implemented by utreexod, but not the original bitcoind client. Click the method name for further details such as parameter and return information.
...
|3|[getcurrentnet](#getcurrentnet)|Y|Get bitcoin network utreexod is running on.|None|
...
|Description|Dynamically changes the debug logging level.<br />The levelspec can either a debug level or of the form `<subsystem>=<level>,<subsystem2>=<level2>,...`<br />The valid debug levels are `trace`, `debug`, `info`, `warn`, `error`, and `critical`.<br />The valid subsystems are `AMGR`, `ADXR`, `BCDB`, `BMGR`, `utreexod`, `CHAN`, `DISC`, `PEER`, `RPCS`, `SCRP`, `SRVR`, and `TXMP`.<br />Additionally, the special keyword `show` can be used to get a list of the available subsystems.|
|Returns|string|
|Example Return|`Done.`|
|Example `show` Return|`Supported subsystems [AMGR ADXR BCDB BMGR utreexod CHAN DISC PEER RPCS SCRP SRVR TXMP]`|
...
|Parameters|None|
|Description|Get bitcoin network utreexod is running on.|
|Returns|numeric|
...
|Description|*DEPRECATED, for similar functionality see [loadtxfilter](#loadtxfilter)*<br />Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this utreexod instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSExtMethodOverview)<br />
...
utreexod uses standard JSON-RPC notifications to notify clients of changes, rather than requiring clients to poll utreexod for updates.  JSON-RPC notifications are a subset of requests, but do not contain an ID.  The notification type is categorized by the `method` field and additional details are sent as a JSON array in the `params` field.
...
[rpcclient](https://github.com/utreexo/utreexod/tree/master/rpcclient) package.
...
[rpcclient](https://github.com/utreexo/utreexod/tree/master/rpcclient) package to connect with
a utreexod instance via Websockets, issues [getblockcount](#getblockcount) to
...
func main() {
	...
	// generated by utreexod when it starts the RPC server and doesn't already
	// have one.
	utreexodHomeDir := btcutil.AppDataDir("utreexod", false)
	certs, err := ioutil.ReadFile(filepath.Join(utreexodHomeDir, "rpc.cert"))
	if err != nil {
		log.Fatal(err)
	}
...
The following is an example Go application which uses the
[rpcclient](https://github.com/utreexo/utreexod/tree/master/rpcclient) package to connect with
a utreexod instance via Websockets, issues [getblock](#getblock) to retrieve
...
The following is an example Go application which uses the
[rpcclient](https://github.com/utreexo/utreexod/tree/master/rpcclient) package to connect with
a utreexod instance via Websockets and registers for
...
(can be installed with `npm install ws`) to connect with a utreexod instance,
...
// generated by utreexod when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/utreexod/appdata/rpc.cert');
var user = "yourusername";
var password = "yourpassword";


// Initiate the websocket connection.  The utreexod generated certificate acts as
